#!/bin/bash
#
# This script disables, dletes, and/or archives users on the local system.
#
...
usage() {
    ...
    echo "  -a  Creates an archive of the home direcotry with the account(s)." >&2
    exit 1
}

# Make sure the script is being executed with useruser privileges.
if [[ "${UID}" -ne 0 ]]; then
...
# If the user doesn't supply at last one argument, give them help.
if [[ "${#}" -lt 1 ]]; then
    usage
fi
...
for USERNAME in "${@}"; do
    ...
    if [[ "${ARCHIVE}" = 'true' ]]; then
        # Make sure the ARCHIVE_DIR direcotry exists.
        ...
        # Arcive the user's home directory and move it into the ARCHIVE_DIR
        ...
        if [[ -d "${HOME_DIR}" ]]; then
            ...
            echo "${HOME_DIR} does not exists or is not a direcotry." >&2
            exit 1
        fi
    fi
    ...
    if [[ "${DELETE_USER}" = 'true' ]]; then
        ...
        if [[ ! $? ]]; then
           ...
           exit 1
            ...
            echo "This account ${USERNAME} was deleted"
        fi
    else
        chage -E 0 "${USERNAME}"
